...
export class IntegrationsService {
  ...
  private register(adapter: GovernmentAdapter): void {
    ...
    this.logger.log(`Registered adapter: ${adapter.adapterId} (${adapter.regulatorName})`);
  }

  getAdapter(adapterId: string): GovernmentAdapter {
    const adapter = this.registry.get(adapterId);
    if (!adapter) throw new NotFoundException(`No adapter registered for id '${adapterId}'`);
    return adapter;
  }

  getAdapterWithCapability(adapterId: string, capability: AdapterCapability): GovernmentAdapter {
    const adapter = this.getAdapter(adapterId);
    if (!adapter.supportsCapability(capability)) {
      throw new Error(`Adapter '${adapterId}' does not support capability '${capability}'`);
    }
    return adapter;
  }
  ...
  async healthCheckAll(): Promise<Array<{ adapterId: string; status: string; latencyMs: number }>> {
    ...
    return results.map((r, i) => {
      ...
      if (r.status === 'rejected') return { adapterId: id, status: 'down', latencyMs: 0 };
      return { adapterId: r.value.adapterId, status: r.value.status, latencyMs: r.value.latencyMs };
    });
  }
}
