...
export class QaCentralBankAdapter implements GovernmentAdapter {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.config = {
      baseUrl: useSandbox
        ? configService.get('QCB_SANDBOX_URL', 'https://sandbox.api.qcb.gov.qa/v1')
    ...
    return {
      ...
      data: [
        ...
        },
        {
          ...
          summary: 'Immediate freezing of funds for newly designated individuals and entities.',
  ...
  private errorResponse<T = unknown>(httpStatus: number, requestId: string, start: number): AdapterResponse<T> {
    return {
      success: false,
      error: { code: `HTTP_${httpStatus}`, message: `QCB API returned HTTP ${httpStatus}`, retryable: httpStatus >= 500 },
      latencyMs: Date.now() - start,
      requestId,
