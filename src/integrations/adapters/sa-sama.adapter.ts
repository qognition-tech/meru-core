...
export class SaSamaAdapter implements GovernmentAdapter {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // "go live".
    const liveRequested = configService.get('SAMA_SANDBOX') === 'false';
    const credentialsPresent =
      !!configService.get('SAMA_CLIENT_ID') &&
      !!configService.get('SAMA_CLIENT_SECRET');
    ...
    this.config = {
      baseUrl: useSandbox
        ? configService.get(
            'SAMA_SANDBOX_URL',
            'https://sandbox.api.sama.gov.sa/v1',
          )
  ...
  private errorResponse<T = unknown>(
    ...
    return {
      ...
      error: {
        ...
        message: `SAMA API returned HTTP ${httpStatus}`,
